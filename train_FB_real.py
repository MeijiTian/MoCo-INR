...
def main() -> None:
    ...
    save_root = Path(config["evaluation"]["save_path"])
    ...
    # -------------------- load data --------------------
    kdata = sitk.GetArrayFromImage(sitk.ReadImage(os.path.join(data_path, "kdata.nii.gz")))  # (T
    smap = sitk.GetArrayFromImage(sitk.ReadImage(os.path.join(data_path, "csm.nii.gz")))          # (C, H, W)

    n_coil, H, W = smap.shape
    n_frame = kdata.shape[0]
    grid_size = kdata.shape[-1]  # keep your original naming

    samp_mask_np = sitk.GetArrayFromImage(sitk.ReadImage(os.path.join(data_path, "sam_mask.nii.gz")))
    ...
    kdata = torch.from_numpy(kdata).to(device)               # (T, C, H, W) complex
    ...
    zf = ifftnc(kdata, dim=(-2, -1))
    zf = torch.sum(zf * smap_t.unsqueeze(0).conj(), dim=1)  # (T, H, W)
    zf = zf / torch.abs(zf).max().clamp_min(1e-12)
    save_nii(
        torch.abs(zf).detach().cpu().numpy(),
        str(recon_dir / f"ZF_recon.nii.gz"),
    )
    ...
    model = MoCoINR(
        config,
        n_frame=n_frame,
        img_size=(kdata.shape[-2], kdata.shape[-1]),
        device=device,
    ).to(device)
    ...
    for epoch in pbar:
        ...
        # normalize
        img_mask = torch.ones_like(smap_t[0])  # dummy mask, keep your behavior
        img_mask[torch.abs(smap_t[0]) == 0] = 0
        
        est_recon_full = est_recon_full / torch.abs(est_recon_full).max().clamp_min(1e-12)
        est_recon_full = est_recon_full * img_mask.unsqueeze(0)
